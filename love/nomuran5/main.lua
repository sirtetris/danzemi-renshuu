...
function Player:new(xPos, yPos)
    local o = MovingThing:new(xPos, yPos, 52, 78, 5, 0.5, 30, false, false)
...
function Player:draw()
    love.graphics.draw(images[self.currImgKey], self.xPos, self.yPos, 0, 0.09, 0.09)
    self.hitBox:draw()
end
...
function Enemy:new(xPos, yPos)
    local o = MovingThing:new(xPos, yPos, 100, 100, 5, 0.5, 30, false, false)
...
function Enemy:draw()
	love.graphics.setColor(200,200,200)   -- 色を設定
    love.graphics.circle("fill", self.xPos+self.w/2, self.yPos+self.h/2, 50,50) -- 長方形を描く
    self.hitBox:draw()
end
...
entities = {}
images = {}
player = Player:new(20, 180)
table.insert(entities, player)
enemy = Enemy:new(700,10)
table.insert(entities,enemy)
egg1 = Egg:new(340, 280)
table.insert(entities, egg1)
floor = Block:new(50, 710, 1200, 5, {200, 200, 200})
plattform1 = Block:new(500, 620, 200, 5, {100, 200, 100})
plattform2 = Block:new(600, 650, 200, 5, {100, 100, 200})
...
plattform7 = Block:new(0,150,1700,5,{50,100,200})
...
table.insert(entities, plattform7)
-- ### /globals
...
function love.update(dt)
    ...
    if player.xPos < enemy.xPos then enemy:accLeft()
    end
    if player.xPos > enemy.xPos then enemy:accRight()
    end
    if player.yPos < enemy.yPos then enemy.ySpeed = -0.5
    end
    if player.yPos > enemy.yPos then enemy.ySpeed = 0.5
	end
end
